...
// - 行の見た目（RowInner）は DnD 非依存にして、リスト本体と DragOverlay の
//   ゴーストの両方で再利用する（virtua がスクロールで元の行を unmount しても
//   DragOverlay のゴーストは消えないようにするため）。
...
// 行ラッパーの共通スタイル（リスト本体・オーバーレイで共有）
const rowRootSx = (item: PlaylistItemType, isActive: boolean): SxProps<Theme> => ({
  display: 'flex',
  alignItems: 'center',
  padding: '2px 4px',
  ...
  cursor: item.exists === false ? 'not-allowed' : 'pointer',
});

// 行の内側（ドラッグハンドル・ファイル名・時間・削除ボタン）。DnD 非依存。
const RowInner: React.FC<{
  item: PlaylistItemType;
  isActive: boolean;
  isDragging?: boolean;
  onRemove?: (id: string) => void;
}> = ({ item, isActive, isDragging, onRemove }) => (
  <>
    <Box
      sx={{
...
const PlaylistItem: React.FC<PlaylistItemProps> = ({ item, index, isActive, onSelect, onRemove }) => {
  ...
  return (
    <Box
      ref={setNodeRef}
      style={{
        // ドラッグ中の本体は DragOverlay のゴーストが追従するので、ここでは
        // カーソル追従 transform を当てず元のスロットに薄く残す（プレースホルダ）。
        transform: isDragging ? undefined : CSS.Transform.toString(transform),
        transition,
      }}
      sx={{
        ...rowRootSx(item, isActive),
      ...
      {...listeners}
    >
      <RowInner item={item} isActive={isActive} isDragging={isDragging} onRemove={onRemove} />
    </Box>
  );
};

// DragOverlay 用のゴースト。DnD 非依存・常に不透明で、カーソルに追従する。
export const PlaylistItemOverlay: React.FC<{ item: PlaylistItemType; isActive: boolean }> = ({ item, isActive }) => (
  <Box
    sx={{
      ...rowRootSx(item, isActive),
      // リストの上に重なるので背景と影で浮かせる
  ...
  >
    <RowInner item={item} isActive={isActive} isDragging />
  </Box>
);
