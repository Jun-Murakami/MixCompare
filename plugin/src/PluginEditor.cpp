    ...
    juce::WebBrowserComponent::NativeFunctionCompletion completion)
{
    ...
    if (action == "apply_layout" && args.size() >= 3)
    {
        ...
        //  ratio は 1 に固定し、constrainer は素の設計 min/max（論理px=CSS px）を使う。
        webResizeRatioW = 1.0;
        webResizeRatioH = 1.0;
        resizerConstraints.setSizeLimits(kDesignMinW, kDesignMinH, kDesignMaxW, kDesignMaxH);
        initialLayoutApplied = true;

        const double dpr = (args.size() >= 4) ? (double)args[3] : 0.0;
        if (auto* pp = getPeer())
        {
            ...
            if (dpr > 0.0 && cssW > 0.0 && peerScale > 0.0 && getWidth() > 0)
            {
                ...
                if (std::abs(F - 1.0) > 0.01)
                {
                    ...
                    mc::cacheWebViewScaleCorrection(newG);
