...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target MixCompare_VST3 MixCompare_AU MixCompare_Standalone MixCompare_CLAP MixCompare_AAX
else
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target MixCompare_VST3 MixCompare_AU MixCompare_Standalone MixCompare_CLAP
fi
...
if [[ -f "${ARTIFACTS_DIR}/CLAP/MixCompare.clap/Contents/MacOS/MixCompare" ]]; then
    echo "  CLAP:"
    lipo -info "${ARTIFACTS_DIR}/CLAP/MixCompare.clap/Contents/MacOS/MixCompare"
fi
...
SRC_CLAP="${ARTIFACTS_DIR}/CLAP/MixCompare.clap"
...
5. For CLAP Plugin:
   Copy the entire MixCompare.clap folder to the following location:
...
#   という自前 DB で管理する（UAD 等もこの方式）。ここへ Utility を seed する。
...
# 実際のプラグインコード(type/subtype/manufacturer)から tagset ファイル名(hex)を算出
...
LOGIC_CATEGORY="${LOGIC_CATEGORY:-Utility}"
echo_step "AU Logic category seed: ${LOGIC_CATEGORY} (tagset=${TAGSET_NAME})"

# postinstall 本体（プレースホルダを後段の sed で実値へ置換。ランタイム変数の誤展開を避けるため引用ヒアドキュメント）
cat > "${SCRIPTS_AU}/postinstall" <<'POSTINSTALL'
#!/bin/sh
# MixCompare: Logic/GarageBand/MainStage のプラグインカテゴリ DB に既定カテゴリを seed。
#  破壊編集を避けるための多重ガード:
...
PLIST
[ -s "$TMP" ] || { rm -f "$TMP" 2>/dev/null; exit 0; }            # 書けていなければスキップ
...
# プレースホルダを実値へ置換（TAGSET_NAME は hex+ハイフン、CATEGORY は単語なので sed 安全）
/usr/bin/sed -i '' \
...
PKG_CLAP="${PKG_WORK_DIR}/MixCompare_CLAP.pkg"
pkgbuild \
    --root "${PKGROOT_CLAP}" \
...
fi
{
    ...
    echo "  </choices-outline>"
    # 注意: selected/enabled は「選択変化のたびに再評価される JS 式」のため、
    #       定数 "true" を渡すと毎回 ON へ戻され、個別にチェックを外せなくなる。
    #       初期状態のみを指定する start_selected/start_enabled を使うこと。
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ZIP_NAME="MixCompare_${VERSION}_macOS_VST3_AU_AAX_CLAP_Standalone.zip"
else
