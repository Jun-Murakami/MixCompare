...
[Setup]
AppId={{5C6E4FB0-9F62-11F0-957F-005056928F3B}
AppName={#MyAppName}
...
OutputBaseFilename=MixCompare_{#MyAppVersion}_Windows_Setup
...
Source: "docs\UserManual.pdf"; DestDir: "{code:GetDocPath}"; Flags: ignoreversion skipifsourcedoesntexist
...
Type: filesandordirs; Name: "{localappdata}\MixCompare"
Type: filesandordirs; Name: "{userappdata}\MixCompare"
...
begin
  ...
  PathSelectionPage.Values[0] := ExpandConstant('{autopf}\MixCompare');
  ...
  begin
    ...
    begin
      if MsgBox('Microsoft Visual C++ Redistributables are required but not installed.' + #13#10 + #13#10 +
                ...
                '(Required for MixCompare to run properly)', mbConfirmation, MB_YESNO) = IDYES then
      begin
        DownloadPage.Clear;
        ...
        try
          ...
          MsgBox('Failed to download Visual C++ Redistributables.' + #13#10 +
                 'MixCompare may not work properly without them.' + #13#10 + #13#10 +
      ...
      begin
        MsgBox('Warning: MixCompare may not work without Visual C++ Redistributables.' + #13#10 + #13#10 +
...
begin
  ...
  begin
    ...
    TestFile := TargetPath + '\MixCompare.vst3\Contents\x86_64-win\MixCompare.vst3';
    ...
    begin
      ...
      begin
        ...
        if MsgBox('The VST3 plugin appears to be in use by a DAW.' + #13#10 + #13#10 +
                  'Please close any DAW that might be using MixCompare and try again.' + #13#10 + #13#10 +
  ...
  begin
    ...
    TestFile := TargetPath + '\MixCompare.clap';
    if FileExists(TestFile) then
    begin
      FileLocked := not DeleteFile(TestFile);
      if FileLocked then
  ...
  begin
    ...
    if DirExists(TargetPath + '\MixCompare.aaxplugin') then
    begin
      Log('Removing existing AAX bundle to avoid nested installation');
      if not DelTree(TargetPath + '\MixCompare.aaxplugin', True, True, True) then
      begin
  ...
  begin
    ...
    if StandalonePath <> '' then
      ...
      TargetPath := ExpandConstant('{autopf}\MixCompare');
      
    TestFile := TargetPath + '\MixCompare.exe';
    if FileExists(TestFile) then
    begin
      FileLocked := not DeleteFile(TestFile);
      if FileLocked then
      begin
        ...
        if MsgBox('MixCompare standalone application appears to be running.' + #13#10 + #13#10 +
                  'Please close it and try again.' + #13#10 + #13#10 +
                  'Continue anyway? (Installation may fail)', mbConfirmation, MB_YESNO) = IDNO then
        begin
          Result := 'MixCompare.exe is running. Please close it and try again.';
          Exit;
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    // Notify user about VST3 scanning
    // if IsComponentSelected('vst3') then
    // begin
    //   MsgBox('VST3 plugin has been installed.' + #13#10 +
    //          'Please scan for new plugins in your DAW to make it available.', mbInformation, MB_OK);
    // end;
    
    // Notify user about AAX
    // if IsComponentSelected('aax') then
    // begin
    //   MsgBox('AAX plugin has been installed.' + #13#10 +
    //          'Please scan for new plugins in Pro Tools to make it available.' + #13#10 + #13#10 +
    //          'Note: If the plugin was not digitally signed, you may need to use' + #13#10 +
    //          'Pro Tools Developer Edition or hold Shift+Ctrl while starting Pro Tools.', mbInformation, MB_OK);
    // end;
  end;
end;
...
begin
  if StandalonePath = '' then
    Result := ExpandConstant('{autopf}\MixCompare')
  else
...
begin
  ...
  DelTree(ExpandConstant('{localappdata}\MixCompare'), True, True, True);
  DelTree(ExpandConstant('{userappdata}\MixCompare'), True, True, True);
end;