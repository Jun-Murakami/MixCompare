...
$BuildTargets = @("MixCompare_VST3", "MixCompare_Standalone", "MixCompare_CLAP")
if ($BuildAAX) { $BuildTargets += "MixCompare_AAX" }
...
$SourceCLAP = "$BuildDir\plugin\MixCompare_artefacts\$Configuration\CLAP\MixCompare.clap"
...
4. For CLAP Plugin:
   Copy MixCompare.clap to the following location:
...
if ($BuildAAX) {
    $ZipName = "MixCompare_${Version}_Windows_VST3_AAX_CLAP_Standalone.zip"
} else {
...
Write-Host "[✓] MixCompare.clap (CLAP plugin)" -ForegroundColor Green
if ($BuildAAX) {
